...
const BackOffice = {
  ...
  try {
    ...
    // รอนานขึ้นเพื่อให้ Firebase sync เสร็จ
    setTimeout(() => {
      this.loadProductsList();
      POS.refresh();
    }, 2000); // เพิ่มจาก 1000 เป็น 2000
    
  } catch (error) {
    Utils.hideLoading(); // ต้องแน่ใจว่า hide loading ทุกกรณี
    console.error("Save product error:", error);
    
    // ถ้าเป็น rate limit ให้แจ้งผู้ใช้
    if (error.code === 'resource-exhausted' || error.message?.includes('resource-exhausted')) {
      Utils.showToast("ระบบถูกใช้งานมากเกินไป กรุณารอ 30 วินาทีแล้วลองใหม่", "warning");
      
      // ปิด modal และ reload อยู่ดี แม้จะ error (เพราะข้อมูลอาจจะบันทึกใน local แล้ว)
      this.closeProductModal();
      setTimeout(() => {
        this.loadProductsList();
      }, 2000);
  ...
  loadMembersList() {
    ...
    members.forEach((member) => {
      ...
      tr.innerHTML = `
                ...
                <td class="p-3 text-center" style="min-width: 120px;">
  <div class="flex justify-center gap-1">
    <button type="button" onclick="event.stopPropagation(); BackOffice.editMember(${member.id})" class="inline-flex items-center justify-center w-10 h-10 text-blue-600 hover:text-blue-700 hover:bg-blue-50 rounded-lg transition">
      <i class="fas fa-edit"></i>
    </button>
    <button type="button" onclick="event.stopPropagation(); BackOffice.deleteMember(${member.id})" class="inline-flex items-center justify-center w-10 h-10 text-red-600 hover:text-red-700 hover:bg-red-50 rounded-lg transition">
      <i class="fas fa-trash"></i>
    </button>
  </div>
</td>
